...
public class ThreeDMinimap : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < Collectibles.Length; i++)
        {
            ...
            g.transform.position = Camera.main.transform.position + Camera.main.transform.forward * forward + Camera.main.transform.right * right + g2.transform.position * scale;
        }
    }
}
